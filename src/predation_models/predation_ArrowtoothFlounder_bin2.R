...
##' @details Predator considered: Arrowtooth Flounder bin 2
...
proj_dir <- here()
output_dir <- paste0(here("model output", "PESC", "Arrowtooth_Flounder_bin2"), "/")
...
DF = read.csv( paste0( data_dir, "Data_ArrowtoothFlounder_bin2_VAST.csv" ) )
...
for (i in seq(n_batches)) {
  ...
  for (j in seq(batch_n)) {
    ...
    ## convert from km2 to hectares: 1 km2 = 100 hectares
